...
/// \class SProcessedDataInput
/// \brief Container for run input information
class SCIXTRACER_EXPORT SProcessedDataInput: public SxContainer{
...
class SCIXTRACER_EXPORT SProcessedDataOutput: public SxContainer{

public:
    SProcessedDataOutput();
    ~SProcessedDataOutput();
...
public:
    ...
    SProcessedDataOutput get_run_output();
    ...
    SProcessedDataInput get_run_input(qint8 index);
...
public:
    ...
    void set_run_input(const QString& name, const SProcessedDataInput& input);

    /// \brief Set the run output. Only one ouput possible which is this processed data
    /// \param[in] output Container for a run output
    void set_run_output(const SProcessedDataOutput& output);

private:
    QString m_run_uri;
    QMap<QString, SProcessedDataInput> m_inputs;
    SProcessedDataOutput m_output;
};
